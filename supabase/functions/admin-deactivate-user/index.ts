...
Deno.serve(async (req) => {
  try {
    ...
    if (body.user_id === callerId) return json(400, { message: "Cannot deactivate yourself" });

    const ownerId = caller.business_owner_id ?? caller.id;

    const { data: target } = await admin
      .from("users")
      .select("id, business_owner_id, is_active")
      .eq("id", body.user_id)
      .single();
    ...
    const { error } = await admin.from("users").update({ is_active: false }).eq("id", body.user_id);
    if (error) return json(400, { message: error.message });

    // Optional: prevent future sign-ins / refresh by banning the auth user long-term.
    // If you prefer a softer disable, remove this line and just rely on is_active in your app.
    await admin.auth.admin.updateUserById(body.user_id, { ban_duration: "876000h" }); // ~100 years

    return json(200, { message: "User deactivated", user_id: body.user_id });
  } catch (e) {
    console.error(e);
